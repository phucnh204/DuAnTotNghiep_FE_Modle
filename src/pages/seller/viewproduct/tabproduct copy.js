...
const TabProduct = () => {
    ...
    return (
        <div style={{ borderRadius: '7px' }} className="container mx-auto p-4 relative bg-white">
            {/* Tabs */}
            <p className="font-semibold">Kênh người bán > Quản lý sản phẩm > Xem sản phẩm</p>
                ...
                <nav className="flex space-x-8 relative" aria-label="Tabs">
                    ...
                    {['tab1', 'tab2'].map((tabId, index) => (
                        <span
                            ...
                            className={`cursor-pointer text-base tab-item text-gray-${
                                activeTab === tabId ? '900' : '500'
                            } py-4 px-4 font-medium relative`}
                            onClick={(e) => switchTab(e, tabId, index)}
                        >
                            {tabId === 'tab1' && 'Đang hoạt động'}
                            {tabId === 'tab2' && 'Ngừng hoạt động'}
                        </span>
                    ))}
                </nav>
                {/* Underline */}
