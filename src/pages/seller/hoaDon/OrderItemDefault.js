...
const OrderItemDefault=({order,isCheckTab})=>{
    ...
    id={'order'+order.id}>
       ...
       <div className="flex space-x-2 p-4 w-12/12 justify-between">
           <div>
               ...
               {/* {trangThai===5&&<button onClick={()=>{
                    alert("Hello world")
               }} disabled={
                ((new Date() - new Date(order.ngayTaoDon)) / (1000 * 60 * 60)) < 2
            } className="border text-sm border-gray-300 text-black px-4 py-2 rounded-sm lg:ml-2">Hoàn tất đơn</button>} */}
